...
AC-29와 [25번](../../../docs/25-openai-api-and-budget.md)의 OpenAI 단일 경로의 로컬/배포 설정·실제 호출·실패 처리·사용량·시연 예산을 검증한다. 최종 모델 설정이 바뀌면 관련 평가와 G5/G6 증거를 갱신한다. 실제 한도를 고갈시키지 말고 소진·조회 불가·429는 모의 오류로 주입한다.
