...
4. dev 표본·고정 regression·로컬 도메인 불변식과 서버 출력 검증으로 빠르게 거른 뒤 통과 후보만 동일 validation 사례·반복수·설정에서 기준선과 paired 비교한다.
...
[25번](../../../docs/25-openai-api-and-budget.md)에 따라 OpenAI 사용량·필수 평가·시연 예산을 확인한다. 모델 변경은 동일 데이터·조건에서 재평가하고 기존 모델의 PASS를 재사용하거나 후보 한도를 초기화하지 않는다. 한도 부족을 다른 제공자/fixture 성공으로 바꾸지 않는다.
