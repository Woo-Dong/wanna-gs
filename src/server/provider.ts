...
import {estimateModelCost,reasoningOptions} from './model-profile';
...
export const liveProvider:ModelProvider=async(instructions,input,schema,name)=>{
  ...
  try{
    ...
    const response=await client.responses.create({model:requestedModel,instructions,input,store:false,...reasoningOptions(requestedModel),max_output_tokens:3200,text:{format:{type:'json_schema',name,strict:true,schema:jsonSchema}}});
    ...
    if(Number.isSafeInteger(inputTokens)&&Number.isSafeInteger(outputTokens)&&Number.isSafeInteger(totalTokens)&&inputTokens!==undefined&&outputTokens!==undefined&&totalTokens!==undefined&&inputTokens>=0&&outputTokens>=0&&totalTokens===inputTokens+outputTokens){
      const estimatedCostUsd=estimateModelCost(response.model,inputTokens,outputTokens);
