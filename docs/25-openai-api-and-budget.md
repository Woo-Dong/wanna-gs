...
D-46(2026-09-21 사용자 직접 승인)에 따라 goal 평가의 로컬 추정 사용비용 soft stop은$20다. 종전$15의 실패·장부·동결 config는 역사적 증거로 남긴다. 신규 실행은 승인된 상한을 실행기/시험에 반영한 새 결속을 사용한다. 전체2400호출, 과거 prior50, unknown시도$.05, 필수 후속 예약은 유지한다. 서버 인스턴스별 기존$15 제한은 다른 제한이며 이번 goal 집계 변경으로 제거하지 않는다. 실제 계정 hard cap·잔액·구매/충전과 구분한다.
