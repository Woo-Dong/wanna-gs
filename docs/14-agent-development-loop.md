...
| C: 고객 요청 | 상품 확인·동의·요청 생성/수정/취소 | F, 관련 제품 결정 | 재전송, 동의 변경, 오래된 제안 |
| S: 상품 식별 | 후보 검색·질문·미식별·실제 모델 | F, 후보 출력 계약 | 정답/모호/없음, 스키마, eval |
| M: 경영주 | 미확보 묶음·자연어 수정·승인 정책 | F/C, 정책 계약 | 이번만/지속, 범위 제한, 오래된 제안 |
| O: 발주 | 최소량·묶음·예산·수요 연결 | F/C, M의 제안 계약 | 초과 금지, 중복 발주, 예산 재검증 |
| R: 이행 | 공급·FIFO 배정·모의 결제·예약·입고·48시간 수령 | F/C/O, 정책 결정 | 순차 배정, 실패/재시도, 정확한 기한 |
...
- `SEED-READY`: schema/migration 뒤 재현 가능한 seed가 실제 DB에서 검증되고, 200개 이상 상품·8~12개 검증 점포 좌표·합성 actor/scenario·eval split/lineage가 같은 버전으로 묶인 상태. 데이터가 존재한다는 주장만으로 G2를 통과하지 않는다.
...
| G2 영역 통합 | 서비스+실제 테스트 SQLite, route+권한, 어댑터 | 제약/rollback/중복/순차 상태 변경/세션 격리, 실제 계약 일치 | 영역 간 연결 |
