...
> **과거 안내 — D-44으로 현재 구현 범위에서 제외.** Neon 계정·프로젝트에 관한 당시 기록이다. 이번 goal에서 아래 설치·연결·배포 명령을 실행하지 않는다. 현재 준비는 [29번 브라우저 SQLite](29-browser-sqlite-demo.md)와 18번 P06/P07을 따른다. 기존 Neon 자원을 삭제하거나 변경하는 지시는 아니다.
