...
| 내 요청 상세 | 상태 배지 → 한 줄 설명 → 상품·수량·점포 → 단계·최근 갱신 → 가능한 행동 | SQLite 상태로 타임라인 구성. 미래 단계는 대기로 구분 |
...
- 상태·기한·결제 결과·제한 사유는 로컬 업무 서비스의 정형 데이터에 연결된 카피 템플릿으로 표시한다. LLM이 성공 상태·숫자·기한을 새로 지어내지 않게 한다. 숫자 3명이나 예시 마감일을 고정하지 않는다.
...
| UX-B03 | 요청 접수/발주 승인만 된 상태를 예약/픽업으로 오해하게 하지 않는가 | 상태별 카피와 로컬 이벤트 대조, 실패·지연 주입 포함 |
| UX-B04 | 모의 결제 실패·제한·만료에서 상태와 가능한 행동을 이해할 수 있는가 | 정상/실패 대조, 잘못된 성공 제목·근거 없는 사유 없음 |
| UX-B05 | 48시간의 시작·마감·종료 상태가 정확한가 | 픽업 알림 생성+48h, 공통 데모 clock 기준 직전/정각/직후, 재전송 시 불변 |
...
| 04 픽업 | 수령 가능·상품/점포·명확한 마감 | 예약/모의 결제/입고 상태 연결. 날짜는 로컬 픽업 알림 생성+48시간에서 계산 | UX-B03~05, AC-12~15 |
...
2. **영역별 FE 구현:** 해당 화면과 공통 토큰/컴포넌트만 소비한다. 홍보 그림을 배경으로 깔고 투명 버튼을 얹는 방식은 사용하지 않는다. 데이터·문구·입력·버튼은 실제 DOM과 SQLite 상태에 연결한다. BE/DB 담당에게 이미지 전체를 매 반복 전달할 필요는 없다.
