...
이 문서 묶음은 지금까지의 사용자 논의를 실행 가능한 설계로 정리한 것이다. 실제 GS 서비스·상품·재고·발주·결제 시스템과 연결하지 않는다. 아직 앱, DB, Vercel 리소스, LLM 연결을 생성하거나 검증한 상태가 아니다. `/goal`도 아직 실행하지 않았다.
...
[27 서비스 가치·불편 방지](27-service-values-and-guardrails.md)는 D-37·CORE-25·AC-31을 구현·회귀 검사에 연결한다. D-36/38의 입력 예시·캐릭터는 26번에 있다. 니즈/추천/거래의 구분, 경영주 업무 감소, 본부 데이터의 향후 가치를 원장과 함께 유지한다.
