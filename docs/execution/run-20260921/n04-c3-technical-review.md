...
| src/server/prompts.ts | `cb448edda6f533144646647a751aa9e7086f6d31509fb14102fe502cca833a1f` |
...
최종 prompt SHA `cb448edda6f533144646647a751aa9e7086f6d31509fb14102fe502cca833a1f`를 직접 재계산했다. 추가 문장 “This scope boundary takes priority over catalog matching: at clarificationCount=2 use unidentified with no primary candidates.”는 범위 밖 실제 실행 요청에서 이미 두 번 질문했다면 상품 이름이 일치해도 primary 후보를 만들거나 다시 질문하지 않도록 기존 경계의 우선순위를 명시한다. 정상 범위의 명확한 상품 탐색은 기존 show_candidates 규칙을 유지한다. 새로운 기능·허용 범위·합격 기준 변경이 아니다.

의미 delta PASS. 실제 모델 실행은 여전히0이고 이 문장이 모델에서 준수되는지는 실제 후보 평가와 구분한다. 앞선25개 서버 테스트를 문장 추가 후 재실행했다고 주장하지 않는다. 이관본의 표만 최종 해시로 갱신했으며 `docs/n04-c3-technical-review.md`의 이전 원본은 수정하지 않고 보존했다. 신규 context-v9 및 전체 gate 지문 ACK는 아직 발급하지 않았다.
