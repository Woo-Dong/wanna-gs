...
전달받은 12,456 입력/3,200 출력 크기를 그대로 대입하면 $0.0101024다. 입력이 같고 출력 300이면 $0.0054624다. 이 계산은 C3 실제 사용량 예측이나 최대 보장이 아니다. 기존 미래 예약 $0.008/attempt는 첫 예시보다 작으므로, 후보 승인 전에 현재 shared ledger와 남은 필수 호출 수를 사용해 예약을 다시 계산해야 한다. 기존 2,400호출/$15, prior reserve 50, unknown 보수 예약 정책은 확대하지 않는다. 현재 예산 잔액은 이번 조사에서 조회하거나 변경하지 않았다.
