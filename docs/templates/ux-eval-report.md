...
- SQLite 상태·event와 화면 일치 / 다른 세션 격리:
...
- stale·상충 조건·중복 승인·부분 실패·예산 재검증·재개 회복:
...
- 상태별 카피 ↔ 로컬 상태·기한 / 정상·실패·대기·기한 종료 증거:
...
픽셀 유사도나 생성 이미지 한 장은 기능 PASS 근거가 아니다. 상태·행동·SQLite/모델 결과와 함께 판정한다.
