...
export class LiveBudgetV3 {
 config:any;token='';authorizationHash='';attempts=0;stopped:string|null=null;trialFailed=false;observations:any[]=[];
 ...
 trialBegin(index:number){this.trialFailed=false;this.call('trial.begin',{index})}
 trialEnd(status:string){if(!this.stopped)this.call('trial.end',{status})}
 stop(code:string){this.stopped=code;try{this.call('stop',{})}catch{}}
 async forward(route:any){if(this.stopped||this.trialFailed){await route.abort();return null}let attemptId:string;const reservedAt=performance.now();
  try{attemptId=this.call('reserve',{}).attemptId;this.attempts++}catch(error:any){this.stop(error.message);await route.abort();return null}
  ...
  try{result=this.call('finish',{attemptId,response:body,httpStatus});this.observations.push({attemptId,httpStatus,...result.observation});this.trialFailed=result.trialFailed;if(result.stopCode)this.stopped=result.stopCode}catch{this.stop('UNCERTAIN_LEDGER_FINISH_STOP');this.observations.push({attemptId,httpStatus,status:'unknown',usage:null,provider_called:null,cost_usd:null,pending:true})}
 ...
 complete(_success:boolean){return this.call('complete',{})}
}
